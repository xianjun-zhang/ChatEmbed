...
export const DefaultAvatar = (props: DefaultAvatarProps) => {
  const isUserMessage = () => props.messageType === 'userMessage';
  const isAIMessage = () => props.messageType === 'apiMessage' || props.messageType === 'leadCaptureMessage';
  ...
  return (
    ...
    >
      <Show when={isUserMessage()}>
        <img src={userIcon} alt="User avatar" class="rounded-full object-cover w-full h-full" />
      </Show>
      <Show when={isAIMessage()}>
        <img src={aiIcon} alt="AI avatar" class="rounded-full object-cover w-full h-full" />
      </Show>
      <Show when={!isUserMessage() && !isAIMessage()}>
        <svg
          width="75"
          height="75"
          viewBox="0 0 75 75"
